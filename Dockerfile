...
COPY . ./
# Build the server
RUN --mount=type=cache,id=9359a4f4-b2f0-4c86-8385-fafa52355594-github-mcp-server-gobuildcache,target=/root/.cache/go-build CGO_ENABLED=0 go build -ldflags="-s -w -X main.version=${VERSION} -X main.commit=$(git rev-parse HEAD) -X main.date=$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
